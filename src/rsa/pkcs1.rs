...
const ASN1_OID_MD2: &'static [u8] = &[0x30, 0x20, 0x30, 0x0c, 0x06, 0x08, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x02, 0x02, 0x05, 0x00, 0x04, 0x10];
const ASN1_OID_MD5: &'static [u8] = &[0x30, 0x20, 0x30, 0x0c, 0x06, 0x08, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x02, 0x05, 0x05, 0x00, 0x04, 0x10];
const ASN1_OID_SHA1: &'static [u8] = &[0x30, 33, 0x30, 9, 0x06, 5, 0x2b, 0x0e, 0x03, 0x02, 0x1a, 0x05, 0, 0x04, 20];
const ASN1_OID_SHA224: &'static [u8] = &[0x30, 0x2D, 0x30, 0x0D, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x04, 0x05, 0x00, 0x04, 0x1C];
const ASN1_OID_SHA256: &'static [u8] = &[0x30, 49, 0x30, 13, 0x06, 9, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x01, 0x05, 0, 0x04, 32];
const ASN1_OID_SHA384: &'static [u8] = &[0x30, 0x41, 0x30, 0x0D, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x02, 0x05, 0x00, 0x04, 0x30];
const ASN1_OID_SHA512: &'static [u8] = &[0x30, 81, 0x30, 13, 0x06, 9, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x03, 0x05, 0, 0x04, 64];
...
pub fn sign_digest_pkcs1<R: Random>(public: &RsaPublicKey, private: &RsaPrivateKey, digest: &[u8], digest_info: &[u8], random: &mut R, signature: &mut [u8]) -> Result<()> {
    unsafe {
        let mut sig = zeroed();
...
pub fn verify_digest_pkcs1(public: &RsaPublicKey, digest: &[u8], digest_info: &[u8], signature: &[u8]) -> Result<bool> {
    unsafe {
        let mut sig = zeroed();
        let mut msg = vec![0u8; digest.len() + digest_info.len()];
...
pub fn sign_pkcs1<H: Pkcs1Hash, R: Random>(public: &RsaPublicKey, private: &RsaPrivateKey, hash: &mut H, random: &mut R, signature: &mut [u8]) -> Result<()> {
...
pub fn verify_pkcs1<H: Pkcs1Hash>(public: &RsaPublicKey, hash: &mut H, signature: &[u8]) -> Result<bool> {
    let mut dst = vec![0u8; hash.digest_size()];
...
pub fn encrypt_pkcs1<R: Random>(public: &RsaPublicKey, random: &mut R, plaintext: &[u8], ciphertext: &mut [u8]) -> Result<()> {
    unsafe {
        let mut out = zeroed();
...
pub fn decrypt_pkcs1<R: Random>(public: &RsaPublicKey, private: &RsaPrivateKey, random: &mut R, ciphertext: &[u8]) -> Result<Box<[u8]>> {
     unsafe {
        let mut inp = zeroed();
