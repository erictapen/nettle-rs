...
impl<C: Cipher> Ctr<C> {
    ...
    pub fn crypt(&mut self, iv: &mut [u8], dst: &mut [u8], src: &[u8]) {
...
mod tests {
    ...
    fn round_trip_ctr_twofish() {
        ...
        let mut enc = Ctr::with_cipher(enc);
        let mut dec = Ctr::with_cipher(dec);
        ...
        enc.crypt(&mut iv1, &mut tmp, &input);
        dec.crypt(&mut iv2, &mut output, &tmp);
