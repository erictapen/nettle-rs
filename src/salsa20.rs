...
impl Salsa20_128 {
    ...
    pub fn with_key_and_nonce(key: &[u8],nonce: &[u8]) -> Self {
        assert_eq!(key.len(), Salsa20_128::KEY_SIZE);
        assert_eq!(nonce.len(), Salsa20_128::NONCE_SIZE);
    ...
    pub fn crypt(&mut self, dst: &mut [u8], src: &[u8]) {
        ...
        unsafe {
            nettle_salsa20_crypt(&mut self.context as *mut _, dst.len(), dst.as_mut_ptr(), src.as_ptr())
        };
    }
}

/// D.J. Bernstein's Salsa20_256 block cipher.
pub struct Salsa20_256 {
    context: salsa20_ctx,
}

impl Salsa20_256 {
    /// Salsa20_256 block size in bytes.
    pub const BLOCK_SIZE: usize = ::nettle_sys::SALSA20_BLOCK_SIZE as usize;

    /// Salsa20_256 key size in bytes.
    pub const KEY_SIZE: usize = ::nettle_sys::SALSA20_256_KEY_SIZE as usize;

    /// Salsa20_256 nonce size in bytes.
    pub const NONCE_SIZE: usize = ::nettle_sys::SALSA20_NONCE_SIZE as usize;

    /// Create a new instance with `key`.
    pub fn with_key_and_nonce(key: &[u8],nonce: &[u8]) -> Self {
        assert_eq!(key.len(), Salsa20_256::KEY_SIZE);
        assert_eq!(nonce.len(), Salsa20_256::NONCE_SIZE);

        let mut ctx = unsafe { zeroed() };
        unsafe { nettle_salsa20_256_set_key(&mut ctx as *mut _, key.as_ptr()); }
        unsafe { nettle_salsa20_set_nonce(&mut ctx as *mut _, nonce.as_ptr()); }

        Salsa20_256{ context: ctx }
    }
...
mod tests {
    ...
    fn salsa128_set_key_and_nonce() {
        ...
        let _ = Salsa20_128::with_key_and_nonce(&key,&nonce);
    }
    ...
    fn salsa128_round_trip() {
        ...
        let mut enc = Salsa20_128::with_key_and_nonce(&key,&nonce);
        let mut dec = Salsa20_128::with_key_and_nonce(&key,&nonce);
    ...
    fn salsa256_set_key_and_nonce() {
        ...
        let _ = Salsa20_256::with_key_and_nonce(&key,&nonce);
    }
    ...
    fn salsa256_round_trip() {
        ...
        let mut enc = Salsa20_256::with_key_and_nonce(&key,&nonce);
        let mut dec = Salsa20_256::with_key_and_nonce(&key,&nonce);
